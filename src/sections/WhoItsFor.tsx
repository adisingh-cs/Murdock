...
const WhoItsFor: React.FC = () => (
  <section id="who-its-for" className="bg-parchment py-24 md:py-32">
    <div className="mx-auto max-w-[1200px] px-6 md:px-12">
      <motion.span {...reveal} className="font-body font-medium text-[13px] uppercase tracking-[0.12em] text-gold block mb-3">Who This Is For</motion.span>
      <motion.h2 {...reveal} className="font-display font-bold text-[30px] md:text-[42px] text-navy max-w-[700px] mb-12">Built for Everyone Who Touches the Legal System in India.</motion.h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {audiences.map((a, i) => (
          <motion.div key={a.title} {...reveal} transition={{ ...revealTransition, delay: i * 0.1 }} className="bg-white rounded-2xl p-8 border border-warm-gray shadow-sm border-l-[3px] border-l-gold" whileHover={{ y: -5, boxShadow: '0 12px 40px rgba(27,43,75,0.14)' }}>
            ...
            <span className="inline-block px-3 py-1 rounded-full font-body font-medium text-[12px] text-navy" style={{ background: 'rgba(27,43,75,0.07)', border: '1px solid rgba(27,43,75,0.15)' }}>{a.tag}</span>
          </motion.div>
        ))}
      </div>
    </div>
