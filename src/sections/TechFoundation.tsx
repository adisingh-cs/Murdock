    ...
    <div className="mx-auto max-w-[1200px] px-6 md:px-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <motion.div {...reveal} className="glass-light p-8">
          <span className="font-body font-medium text-[13px] uppercase tracking-[0.12em] text-gold block mb-6">
            ...
            {pipelineSteps.map((step, i) => (
              <div key={step} className="flex items-start gap-4">
                <div className="flex flex-col items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[13px] font-body font-medium shrink-0 ${i === 2 ? 'bg-gold text-navy' : 'bg-navy text-white'}`}>
                    {i + 1}
                  </div>
                  {i < pipelineSteps.length - 1 && (
                    <div className="w-px h-6 border-l border-dashed border-warm-gray" />
                  )}
                </div>
                <div className="pb-4 flex items-center gap-2">
                  <span className={`font-body font-medium text-[15px] ${i === 2 ? 'text-gold' : 'text-navy'}`}>{step}</span>
                  {i === 2 && (
                    <span className="px-2 py-0.5 rounded-full text-[11px] font-body font-medium bg-brand-green/10 text-brand-green border border-brand-green/30">
      ...
      <motion.div {...reveal} className="text-center">
        <span className="font-body font-medium text-[13px] uppercase tracking-[0.12em] text-gold block mb-2">
          AI Provider Abstraction
        </span>
        <p className="font-body text-[17px] text-slate mb-6 max-w-[500px] mx-auto">
          Murdock never locks you into one AI model. Switch between providers at runtime — no restart required.
        </p>
        <div className="flex flex-wrap justify-center gap-3">
          {aiProviders.map((p) => (
            <motion.span
              key={p}
              className="px-4 py-2 rounded-full font-body font-medium text-[14px] text-navy bg-white border border-warm-gray cursor-default"
              whileHover={{ borderColor: '#C9933A' }}
            >
              {p}
            </motion.span>
          ))}
